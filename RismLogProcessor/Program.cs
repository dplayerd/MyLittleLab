    ...
    public static class Program
    {
        public static int Main(string[] args)
        {
            try
            {
                ...
                var pipeline = new RismLogProcessingPipeline();
                var result = pipeline.RunFromProjectDir(projectDir);

                Console.WriteLine();
                Console.WriteLine($"完成：{result.OutputPath}");
                Console.WriteLine($"SourceLog: {result.SourceLog.Count}");
                Console.WriteLine($"Only89   : {result.Only89.Count}");
                Console.WriteLine($"Only76   : {result.Only76.Count}");
                Console.WriteLine($"OnlyF6   : {result.OnlyF6.Count}");
                Console.WriteLine($"Only80   : {result.Only80.Count} (已過濾 OK，只保留能配對者)");
                return 0;
